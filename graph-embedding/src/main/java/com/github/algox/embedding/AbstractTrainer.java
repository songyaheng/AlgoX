...
import com.github.algox.common.Task;
...
public abstract class AbstractTrainer {
    ...
    protected static final int MAX_EXP = 6;

    /** Size of the pre-cached exponent table */
    protected static final int EXP_TABLE_SIZE = 1000;
    protected static final double[] EXP_TABLE = new double[EXP_TABLE_SIZE];
    static {
    ...
    protected final NNConfig config;
    protected final Map<String, HuffmanNode> huffmanNodes;
    private final int vocabSize;
    protected final int layer1_size;
    protected final int window;
    /**
     * In the C version, this includes the </s> token that replaces a newline character
     */
    private int numTrainedTokens;
     ...
     */
    protected final double[][] syn0;
    /** This contains hidden layers of the neural network */
    protected final double[][] syn1;
    /** This is used for negative sampling */
    private final double[][] syn1neg;
    /** Used for negative sampling */
    private final int[] table;

    public AbstractTrainer(NNConfig config, Multiset<String> vocab, Map<String, HuffmanNode> huffmanNodes) {
        this.config = config;
    ...
    private void initializeSyn0() {
        ...
        for (int a = 0; a < huffmanNodes.size(); a++) {
            ...
            for (int b = 0; b < layer1_size; b++) {
                nextRandom = incrementRandom(nextRandom);
                syn0[a][b] = (((nextRandom & 0xFFFF) / (double)65536) - 0.5) / layer1_size;
            }
        }
    }
    ...
    public NeuralNetworkModel train(Iterable<List<String>> sentences) {
        ListeningExecutorService ex = MoreExecutors.listeningDecorator(newFixedThreadPool(config.numThread));
        int numSentences = Iterables.size(sentences);
        numTrainedTokens += numSentences;
        // Partition the sentences evenly amongst the threads
        Iterable<List<List<String>>> partitioned = Iterables.partition(sentences, numSentences / config.numThread + 1);
        try {
    ...
    protected abstract Worker createWorker(int randomSeed, int iter, Iterable<List<String>> batch);

    /** Worker thread that updates the neural network model */
    public abstract class Worker extends Task {
        ...
        protected final double[] neu1 = new double[layer1_size];
        protected final double[] neu1e = new double[layer1_size];

        public Worker(int randomSeed, int iter, Iterable<List<String>> batch) {
        ...
        public void run() throws InterruptedException {
            ...
            for (List<String> sentence : batch) {
                ...
                LoggerUtils.info("%s is training batch %d success", Thread.currentThread().getName(), count);
                count ++;
            }
        ...
        protected void handleNegativeSampling(HuffmanNode huffmanNode) {
            for (int d = 0; d <= config.negativeSamples; d++) {
                int target;
                final int label;
                if (d == 0) {
                    target = huffmanNode.index;
                    label = 1;
                } else {
                    nextRandom = incrementRandom(nextRandom);
        ...
        public abstract void trainSentence(List<String> unfiltered);
